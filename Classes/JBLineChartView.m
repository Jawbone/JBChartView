...
@interface JBLineLayer : JBShapeLayer
...
@interface JBFillLayer : JBShapeLayer
...
{
	_animated = animated && self.currentPath != nil;
}
...
- (void)drawRect:(CGRect)rect
{
	...
	{
		UIBezierPath *shapeLayerPath = [self bezierPathForLineIndex:lineIndex filled:NO];
		UIBezierPath *filledShapeLayerPath = [self bezierPathForLineIndex:lineIndex filled:YES];

		if (shapeLayerPath == nil || filledShapeLayerPath == nil)
		{
			continue;
		}
		...
		shapeFillLayer.path = filledShapeLayerPath.CGPath;
...
- (void)reloadDataAnimated:(BOOL)animated callback:(void (^)())callback
{
	...
	for (CALayer *layer in [self.layer sublayers])
	{
		...
		{
			((JBShapeLayer *)layer).animated = animated;
			removeLayer = (((JBLineLayer *)layer).tag >= lineCount);
		}
